...
$data = $api->index((object)[
	'app_srl' => __APP_SRL__,
	'field' => 'srl,title,json,regdate,content',
	'page' => 1,
	'print_data' => 'article',
...
foreach($data->articles as $k=>$o)
{
	...
	{
		$file = externalApi('/files/'.(int)$data->articles[$k]->json->thumbnail->srl);

		$data->articles[$k]->content = (
			'<img src="' . __GOOSE_ROOT__ . '/' . $file->loc . '" alt="' . $file->name . '" style="display:block;margin:0 auto;">'.
			'<p>'.
