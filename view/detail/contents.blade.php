	...
	<article class="article-view" id="articleView">
		...
		<nav class="control">
			...
			<span class="on-like" title="on like">
				<img src="{{ __ROOT__ }}/assets/dist/img/ico-heart-on.svg" alt="">
				<em>{{ $repo['article']['json']['like'] or 0 }}</em>
			</span>
			@else
			<a href="{{ __ROOT__ }}/upLike/{{ $repo['article']['srl'] }}/" class="on-like" title="on like">
				<img src="{{ __ROOT__ }}/assets/dist/img/ico-heart.svg" alt="">
				<em>{{ $repo['article']['json']['like'] or 0 }}</em>
			</a>
			@endif
		</nav>
		...
		<nav class="external-control">
			...
			<a href="{{ __ROOT__ }}/article/{{ $_nest ? $_nest.'/' : '' }}{{ $repo['anotherArticle']['prev']['srl'] }}/" class="direction next">
				<img src="{{ __ROOT__ }}/assets/dist/img/btn-arrow-right2.svg" alt="prev article">
			</a>
			@endif
			@if($repo['anotherArticle']['next'])
			<a href="{{ __ROOT__ }}/article/{{ $_nest ? $_nest.'/' : '' }}{{ $repo['anotherArticle']['next']['srl'] }}/" class="direction prev">
				<img src="{{ __ROOT__ }}/assets/dist/img/btn-arrow-left2.svg" alt="next article">
			</a>
			@endif
			@if($_GET['popup'])
			<button type="button" data-action="close" class="close">
				<img src="{{ __ROOT__ }}/assets/dist/img/btn-close2.svg" alt="close">
			</button>
			@endif
		</nav>
	</article>
